...
MH.register({
  ...
  create: function (sdk, root) {
    ...
    var AUTO_SEED = cfg.autoSeed === true;   // demo: populate on first run instead of empty
    ...
    function buildScaffold() {
      ...
      // Fill the host-provided root. The SDK root is position:absolute; inset:0;
      // keep it that way (a sized, positioned containing block) so .bft-root's
      // inset:0 anchors to a full-height box. Setting position:relative here
      // would collapse root to 0 height (its only child is absolutely positioned).
    ...
    function initMap() {
      ...
      /* If the iframe wasn't laid out yet, computeMapSize() hits its 60px floor
         and all nodes collapse into a tiny cluster. Poll a few frames until the
         container has a real size, then rebuild. (Belt-and-suspenders with the
         ResizeObserver, which can miss the initial 0→full transition.) */
      if (_mapSize <= 60) { scheduleMapResize(0); }
    }

    function scheduleMapResize(tries) {
      if (tries > 30 || typeof requestAnimationFrame === 'undefined') { return; }
    ...
    sdk.store.kv.get('bagfilter', 'state').then(function (saved) {
      ...
      if (saved) {
        ...
        // Demo: auto-populate simulated data so the map is never blank
        _state = buildSeedState();
        initMap();
        startTick();
        startResizeObserver();
        renderActiveTab();
        sdk.store.tx.add({ ns: 'bagfilter', action: 'seed', user: 'auto',
          note: 'Auto-seeded demo data — ' + SLOTS + ' slots' }).catch(function () {});
        sdk.store.kv.put('bagfilter', 'state', _state).catch(function () {});
        sdk.ui.toast('Demo data auto-seeded (' + SLOTS + ' slots)', 'good');
      } else {
